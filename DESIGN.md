...
| Docker deployment | 🔲 Removed after review | Built, then removed once real memory numbers were measured (~400MB app baseline). See decisions log below |
| Web-based knowledge-base setup | ✅ Done | Upload company/product/policy docs from the Overview page's Setup panel - no CLI required, chatbot/normalizer context updates live without a restart |
...
| Docker built, then removed | Was added as the deployment story, then measured: the running server's baseline is ~400MB (mostly the embedding model), and a container runtime adds real overhead on top of that (a Docker Desktop VM alone often reserves 2GB) - too much for the small self-hosted deployments this project targets, for a benefit (process isolation, portability) that a plain Python process + the operator's own supervisor already covers well enough at this scale | Keeping Docker as "the" recommended path anyway (would mislead the target user about what this actually costs to run), building a slimmer custom-compiled image to shrink the overhead (still doesn't remove the daemon/VM cost that was the actual complaint, and adds real maintenance weight for a niche path) |
| No process supervisor bundled (no systemd unit, no Docker restart policy) | Every real deployment target already has its own preferred way to keep a process alive (systemd, pm2, supervisor, their platform's own primitive) - bundling one more is one more opinion this project doesn't need to hold | A bundled systemd unit file (Linux-only, doesn't help Mac/Windows self-hosters, and duplicates something operators already have a working answer for) |
